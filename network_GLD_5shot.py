...
class Conv_64F(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3, batch_size=4, shot_num=5):
...
class GLD_Metric(nn.Module):
    def __init__(self, num_classes=5, neighbor_k=3, batch_size=4, shot_num=5):
        ...
        self.Norm_layer = nn.BatchNorm1d(self.num_classes * 4, affine=True)
        self.FC_layer = nn.Conv1d(1, 1, kernel_size=4, stride=1, dilation=5, bias=False)
    ...
    def cal_GLD_similarity(self, input1_batch, input2_batch):
        ...
        for i in range(self.batch_size):
            ...
            input2_mean = input2.view(b2//self.shot_num,self.shot_num,hw,c).mean(1)

            input1 = input1.permute(0,2,1).view(b1,c,21,21)
            input2_mean = input2_mean.permute(0,2,1).view(b2//self.shot_num,c,21,21)

            input1 = self.m(input1).view(b1,c,5*5).permute(0,2,1)
            input2_mean = self.m(input2_mean).view(b2//self.shot_num,c,5*5).permute(0,2,1)

            sim_matrix = self.calculate_sim_matrix(input1, input2_mean)
            topk_value, _ = torch.topk(sim_matrix, self.neighbor_k, 3)
            inner_sim2 = torch.sum(torch.sum(topk_value, 3), 2)
            
            input1 = input1.permute(0,2,1).contiguous().view(b1,-1)
            input2_mean = input2_mean.permute(0,2,1).contiguous().view(b2//self.shot_num,-1)

            inner_sim3= self.euclidean_metric(input1, input2_mean)
            inner_sim3 = 1/(1 + torch.exp(1e-04*inner_sim3))

            input1 = input1.view(b1,c,5*5).permute(0,2,1)
            input2_mean = input2_mean.view(b2//self.shot_num,c,5*5).permute(0,2,1)

            input1 = input1- input1.mean(-1).unsqueeze(-1)
            input2_mean = input2_mean- input2_mean.mean(-1).unsqueeze(-1)

            sim_matrix = self.calculate_sim_matrix(input1, input2_mean)
            inner_sim4= self.W0(sim_matrix.view(75*self.num_classes,25*25)).view(75,self.num_classes)
            
            sim_soft = torch.cat(( inner_sim1, inner_sim2, inner_sim3, inner_sim4), 1)
